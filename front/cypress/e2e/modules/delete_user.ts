...
export function deleteUser_e2eTest() {
  describe('Delete User Spec', () => {
    ...
    beforeEach(() => {
      interceptRegisterUser();
      interceptLoginUser();
      interceptFetchSessions();
      interceptFetchUserDetails();
      interceptDeleteUser();
    });
  ...
  const interceptFetchUserDetails = () => {
    cy.intercept('GET', 'api/user/8', {
      id: 8,
      email: 'test@test.com',
      lastName: 'User',
      firstName: 'Test',
  ...
  const interceptDeleteUser = () => {
    cy.intercept('DELETE', '/api/user/8', {
      statusCode: 200,
      body: null,
    }).as('deleteUser');
  };
